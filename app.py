...
live_capture = LivePacketCapture(feature_names=feature_names)
...
def get_prediction(model_name, features_array):
    """Run prediction on a single packet using selected model or auto-ensemble"""
    ...
    if model_name == 'auto':  
        # Evaluate across all available models and apply attack-override + weighted voting  
        results = [run_model(m, features_array) for m in available_models]  
        ...
        if override_candidates:  
            _, chosen = max(override_candidates, key=lambda x: x[0])  
            final_pred = 1  
            final_conf = chosen['confidence']  
            decision_mode = "attack_override"  
        else:  
            # 2) Weighted ensemble
            attack_score = 0.0  
            benign_score = 0.0  
            ...
            final_pred = 1 if attack_score > benign_score else 0  
            ...
            for r in results:  
                ...
                if r_binary == final_pred:  
                    ...
                    if score > best_score:  
                        best_score = score  
                        best = r  
            final_conf = best['confidence'] if best is not None else 0.0  

        return {  
            'prediction': int(final_pred),  
            'label': 'BENIGN' if final_pred == 0 else 'ATTACK',  
        ...
        result = run_model(model_name, features_array)  
...
def predict():  
    ...
    try:  
        ...
        result, error = get_prediction(model_name, arr)
        ...
        for i, (_, sample_row) in enumerate(samples.iterrows()):  
            ...
            if model_name == 'auto':  
                ...
                if override_candidates:  
                    ...
                    pred = 1 if attack_score > benign_score else 0  
...
def start_live_capture():
    ...
    try:
        ...
        interface = data.get('interface') # None for auto-detect
        
        # Determine if we should attempt to set interface
        if interface and interface.strip():
            live_capture.interface = interface.strip()
        ...
        return jsonify({
            ...
            'interface': live_capture.interface
        })
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def get_live_results():
    """Get processed features from live capture and run predictions"""
    try:
        model_name = request.args.get('model', 'auto')
        results = []
        
        # Try to pull up to 10 flows at once to avoid overwhelming frontend
        for _ in range(10):
            # timeout=0.1 so we don't block for long if empty
            features_dict = live_capture.get_features(timeout=0.1)
            
            if not features_dict:
                break
                
            # Keep original metadata for response
            flow_key = features_dict.get('flow_key', 'unknown')
            timestamp = features_dict.get('timestamp', time.time())
            
            # Format features for model
            feature_array = live_capture.format_features_for_model(features_dict)
            
            if feature_array is None:
                continue
                
            # Run prediction
            pred_result, error = get_prediction(model_name, feature_array)
            
            if error:
                continue
                
            # Augment result
            pred_result['packet_id'] = flow_key # Using flow key as ID for live representation
            pred_result['timestamp'] = datetime.fromtimestamp(timestamp).isoformat()
            
            # Use same expected response structure as simulate
        ...
        # Also return underlying capture stats
        capture_stats = live_capture.get_statistics()
        ...
        print(f"Error getting live results: {e}")
        return jsonify({'error': str(e)}), 500
